  ...
  onSelectFile = e => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      reader.addEventListener("load", (event) =>{
        ...
        imageObj.onload = ()=> {
            //TEST IMAGE SIZE
            if (imageObj.height < 1199 || imageObj.width < 1199) {
    ...
    if (this.state.title != '' && this.state.esporte != '' && this.state.loja != '' && this.state.tagline != '' && this.state.last != '' && this.state.src != null) {
      ...
      html2canvas(input, {
        ...
        const pdf = new jsPDF({
          ...
          unit: 'pt',
          format:this.state.template ==0? [1020.47, 1899.21]:[1474.1, 4064.2],
        ...
        pdf.save(`ColaboradorEsportista-Banco_${this.state.title}_${this.state.esporte}_36x67.pdf`, { returnPromise: true }).then(this.setState({ loading: false }));
          ...
          pdf.addImage(imgData, 'JPG', 0, 0, 1474.1, 4064.2, 'img', `FAST`)
          pdf.save(`ColaboradorEsportista-Banco_${this.state.title}_${this.state.esporte}_55x150.pdf`, { returnPromise: true }).then(this.setState({ loading: false }));
      ...
      <div className="App" style={{ width: '100%' }} onScroll={this.scroll}>
       ...
       {this.state.template ==0?
        <div id='elementId' style={{ top: -1000, left: 0, position: "fixed", width: 282.8, height: 526.33333 }}>
          ...
          <div id='txtarea' ref='inner' style={{ whiteSpace: 'pre-wrap', wordBreak: "break-word", position: 'absolute', textShadow: this.state.colorText==`#FFF`? 'rgba(0,0,0,0.5) 0.01em 0.08em 1em':'rgba(255,255,255,0.5) 0.01em 0.08em 1em', shadowOpacity: 0.25, marginTop: `${this.state.y - ((this.lineSize))}px`, marginLeft: `${this.state.x}px`, zIndex: 8, marginRight: 12, color: this.state.colorText, fontFamily: 'Roboto Condensed', fontWeight: 600 }}>
            {'<< Pratico '}{this.state.esporte}{" há\nmais de "}{this.state.txtarea}{" anos"}{" >>"}
          </div>
          <img style={{ top: 0, left: 0, position: 'absolute', width: 282.8, height: 526.33333, zIndex: 2 }} src={require("./Images/template.png")} />
          <div style={{ position: 'absolute', zIndex: 3, color: '#0082c3', fontStyle: 'bold', marginLeft: 18, textAlign: `left` }}>

          <p className='tituloteste' style={{ lineHeight: 1, marginRight: 18, wordBreak: "break-word", width: 175, fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize: 26, margin: 0, marginTop: 50, marginBottom: 0, textTransform: 'uppercase' }}>{this.state.title!=''?this.state.title:`NOME`}</p>
                    <p style={{ fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize:10, margin: 0, marginBottom: 0 }}>{`Apaixonado por ` + (this.state.esporte!=''?this.state.esporte:`corrida`)}</p>
                    <p style={{ fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize: 10, margin: 0, marginBottom: 0 }}>a seu serviço!</p>
                    <p style={{ fontSize: 10, margin: 0, marginTop: 2, marginBottom: 0 }}>{'Na sua loja Decathlon ' + (this.state.loja!=''?this.state.loja:`morumbi`)}</p>
        ...
        <div id='elementId' style={{ top: -1000, left: 0, position: "fixed",width: 250, height: 775 }}>
                 
        <div id='txtarea' ref='inner' style={{fontSize:10, whiteSpace: 'pre-wrap', wordBreak: "break-word", position: 'absolute', textShadow: this.state.colorText==`#FFF`? 'rgba(0,0,0,0.5) 0.01em 0.08em 1em':'rgba(255,255,255,0.5) 0.01em 0.08em 1em', shadowOpacity: 0.25, marginTop: `${this.state.y - ((this.lineSize))}px`, marginLeft: `${this.state.x}px`, zIndex: 8, marginRight: 12, color: this.state.colorText, fontFamily: 'Roboto Condensed', fontWeight: 600 }}>
          {'<< Pratico '}{this.state.esporte!=''?this.state.esporte:`corrida`}{" há\nmais de "}{(this.state.txtarea!=''?this.state.txtarea:`5`)}{" anos"}{" >>"}
        </div>
        <img style={{ top: 0, left: 0, position: 'absolute', position: 'absolute', width: 250, height: 775, zIndex: 2 }} src={require("./Images/template2.png")} />
        <div style={{ position: 'absolute', zIndex: 3, color: '#0082c3', fontStyle: 'bold', marginLeft: 45, textAlign: `left` }}>

          <p className='tituloteste' style={{ lineHeight: 1, marginRight: 18, wordBreak: "break-word", width: 175, fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize: 26, margin: 0, marginTop: 50, marginBottom: 0, textTransform: 'uppercase' }}>{this.state.title!=''?this.state.title:`NOME`}</p>
          <p style={{ fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize:10, margin: 0, marginBottom: 0 }}>{`Apaixonado por ` + (this.state.esporte!=''?this.state.esporte:`corrida`)}</p>
          <p style={{ fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize: 10, margin: 0, marginBottom: 0 }}>a seu serviço!</p>
          <p style={{ fontFamily: 'Roboto Condensed',fontSize: 10, margin: 0, marginBottom: 0 }}>{'Na sua loja Decathlon ' + (this.state.loja!=''?this.state.loja:`morumbi`)}</p>
        ...
        <div style={{ height: '95%' }}>
          {this.state.base64Image && (
            ...
            <img alt="Crop" style={{marginLeft:10, width: "90%", maxHeight: '100%', marginTop: `60%` }} src={this.state.base64Image} />)}
        ...
        <div style={{ backgroundColor: `#0082c3`, height: 45, width: this.state.widthPercent, margin: 0 }}onMouseDown={()=> this.setState({template:1})}>
        ...
        <div style={{ marginTop: 40, flex: 1, width: `100%`,textAlign:'left', alignContent: 'center' }}>
          <div className="Body">
            ...
            <div style={{ marginLeft: 20, marginTop: 20, marginBottom: 45 }}>
              ...
              <p onClick={()=> window.open('https://www.mediadecathlon.com/index.html')} style={{color:'#0082c3',cursor:'pointer',margin:0,textDecorationLine:'underline'}}>Escolha uma foto de esporte no Pixl</p>
              <label for='file'  ><img for="file" src={require("./Images/Escolha.jpg")} style={{ width: 275, marginTop: 25 }} /></label>
              ...
              {!!this.state.src &&
                ...
                <Cropper
                  ...
                  // Cropper.js options
                  aspectRatio={this.state.template ==0? 4252 / 4717:6142/16650}
                  guides={false}
              ...
              {this.state.template ==0?
                ...
                <input onChange={(event) => this.setState({ x: event.target.value })} value={this.state.x} type="range" min="27" max={250 - this.lineWidth} ></input>
                <input onChange={(event) => this.setState({ y: event.target.value })} value={this.state.y} type="range" min={238} max={810 - this.lineSize} ></input>
              ...
              <input accept="image/png, image/jpeg" name="file" id="file" type="file" className="inputfile" onChange={this.onSelectFile} />
...
{this.state.template ==0?
                <div className="PDF" style={{ width: 282.8, height: 526.33333 }}>
                  ...
                  <div id='elementId' style={{ width: 282.8, height: 526.33333 }}>
                    ...
                    <div id='txtarea' ref='inner' style={{ whiteSpace: 'pre-wrap', wordBreak: "break-word", position: 'absolute', textShadow: this.state.colorText==`#FFF`? 'rgba(0,0,0,0.4) 0.01em 0.08em 0.2em':'rgba(255,255,255,0.4) 0.01em 0.08em 0.2em', shadowOpacity: 0.25, marginTop: `${this.state.y - ((this.lineSize))}px`, marginLeft: `${this.state.x}px`, zIndex: 8, marginRight: 12, color: this.state.colorText, fontFamily: 'Roboto Condensed', fontWeight: 600 }}>
                      {'<< Pratico '}{this.state.esporte!=''?this.state.esporte:`corrida`}{" há\nmais de "}{(this.state.txtarea!=''?this.state.txtarea:`5`)}{" anos"}{" >>"}
                    </div>
                    <img style={{ position: 'absolute', width: 282.8, height: 526.33333, zIndex: 2 }} src={require("./Images/template.png")} />
                    <div style={{ position: 'absolute', zIndex: 3, color: '#0082c3', fontStyle: 'bold', marginLeft: 18, textAlign: `left` }}>
                      ...
                      <p style={{ fontFamily: 'Roboto Condensed', fontWeight: 700, fontSize: 15, margin: 0, marginBottom: 0 }}>a seu serviço!</p>
                ...
                :
                <div className="PDF" style={{ width: 250, height: 775 }}>

                <div id='elementId' style={{ width: 250, height: 775 }}>
                 
                <div id='txtarea' ref='inner' style={{fontSize:10, whiteSpace: 'pre-wrap', wordBreak: "break-word", position: 'absolute', textShadow: this.state.colorText==`#FFF`? 'rgba(0,0,0,0.5) 0.01em 0.08em 1em':'rgba(255,255,255,0.5) 0.01em 0.08em 1em', shadowOpacity: 0.25, marginTop: `${this.state.y - ((this.lineSize))}px`, marginLeft: `${this.state.x}px`, zIndex: 8, marginRight: 12, color: this.state.colorText, fontFamily: 'Roboto Condensed', fontWeight: 600 }}>
          {'<< Pratico '}{this.state.esporte!=''?this.state.esporte:`corrida`}{" há\nmais de "}{(this.state.txtarea!=''?this.state.txtarea:`5`)}{" anos"}{" >>"}
        </div>
                  <img style={{ position: 'absolute', width: 250, height: 775, zIndex: 2 }} src={require("./Images/template2.png")} />
                  <div style={{ position: 'absolute', zIndex: 3, color: '#0082c3', fontStyle: 'bold', marginLeft: 45, textAlign: `left` }}>
                  ...
                  <div style={{ height: '95%' }}>
                    {this.state.base64Image && (
                      ...
                      <img alt="Crop" style={{marginLeft:17, width: "90%", maxHeight: '100%', marginTop: `60%` }} src={this.state.base64Image} />)}
                ...
                <img onMouseDown={() => this.printDocument()} src={require("./Images/pdf.jpg")} style={{ cursor: 'pointer', width: 180, marginTop: 20 }} />
